...
7. **Mechanical tell scan** — write the draft to the scratchpad first (e.g. `<scratchpad>/insight-draft.md`), then apply the **`plainspoken-prose` skill's Final Check** to it before saving to the library. That skill owns the tell rules and ships the Vale style that enforces them; read its SKILL.md if it isn't already loaded. Its check is currently:
   ```bash
   vale --config ~/.claude/skills/plainspoken-prose/vale/.vale.ini <draft>
   ```
   If that path doesn't resolve (the prose skill moved, or this is running in a harness with a different skills directory), locate the `plainspoken-prose` skill and follow its Final Check as written there — the skill is the source of truth, not this command. Resolve every **error**: rewrite it as a positive claim, unless the contrast is explicitly the speaker's own — in that case keep it but attribute it ("he draws the line between X and Y"). Judge every **warning** by reading the hit (intensifiers and excess-vocab words have legitimate uses). Do not self-assess the draft as fine without running the scan: loading `plainspoken-prose` does not apply it, and the writer that produced a contrast flip will read it as "doing work". The lint is the check; a clean run is the exit condition. (Fallback if `vale` is missing: the rg pattern in the prose skill's Final Check.)
...
When generating the chat reply and saved insight, invoke the `plainspoken-prose` skill for its drafting rules, then run the mechanical tell scan from workflow step 7 over the draft — the scan, not a re-read, is what verifies the prose. Write clear, direct, specific prose without generic AI-style polish. Keep the structure below, but let the sentences sound like a careful human note rather than a template fill.
...
- appositive negation chains ("it was verification-based RL — not RLHF, not just bigger pretraining — that…"): delete the negated clauses and state the affirmative ("verification-based RL is what let…")
